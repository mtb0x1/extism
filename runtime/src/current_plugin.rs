...
pub struct CurrentPlugin {
    /// Plugin variables
    pub(crate) vars: std::collections::HashMap<String, Vec<u8>>,
...
impl CurrentPlugin {
    ...
    pub fn vars(&self) -> &std::collections::HashMap<String, Vec<u8>> {
        &self.vars
    }

    /// Mutable access to a plugin's variables
    pub fn vars_mut(&mut self) -> &mut std::collections::HashMap<String, Vec<u8>> {
        &mut self.vars
    }
